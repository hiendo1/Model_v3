...
def get_team_data(query, league, team_db):
    ...
    1. ID-based Match (if query is digit/ID)
    2. Exact Match: "Team|League"
    3. Exact Name Match
    ...
    """
    ...
    # 1. ID-based Match (if query looks like an ID)
    query_str = clean_id(query)
    if query_str.isdigit() or (query_str.startswith('sr:competitor:') or query_str.startswith('comp_')):
        id_key = f"{query_str}|{league}"
        if id_key in team_db:
            return team_db[id_key], team_db[id_key].get('team_name', query_str)
        if query_str in team_db:
            return team_db[query_str], team_db[query_str].get('team_name', query_str)
            
    # 2. Exact Match (Best case for names)
...
def predict_by_name():
    ...
    try:
        ...
        # 1. Start with H2H
        # Priority 1: ID-based lookup (More stable)
        h_id = h_stats.get('team_id')
        a_id = a_stats.get('team_id')
        
        h2h = h2h_db.get(f"{h_id}|{a_id}") or h2h_db.get(f"{h_name}|{a_name}")
        if not h2h:
            rev_key_id = f"{a_id}|{h_id}"
            rev_key_name = f"{a_name}|{h_name}"
            rev = h2h_db.get(rev_key_id) or h2h_db.get(rev_key_name)
            if rev:
                h2h = {
        ...
        league_id = h_stats.get('league_id')
        unique_league_key = f"{league}|{league_id}" if league_id else league
        
        # Try unique key, fallback to simple league name
        v4_states = model_package.get("v4_state", {})
        state = v4_states.get(unique_league_key, v4_states.get(league, {})) if v4_states else {}
        
        h_id = h_stats.get('team_id')
        a_id = a_stats.get('team_id')
        
        # Priority 1: Lookup by ID, Priority 2: Lookup by Name
        h_elo = state.get("elo", {}).get(h_id, state.get("elo", {}).get(h_name, 1500.0))
        a_elo = state.get("elo", {}).get(a_id, state.get("elo", {}).get(a_name, 1500.0))
        ...
        h_last = state.get("last_date", {}).get(h_id) or state.get("last_date", {}).get(h_name) or h_stats.get('last_match_date')
        a_last = state.get("last_date", {}).get(a_id) or state.get("last_date", {}).get(a_name) or a_stats.get('last_match_date')
